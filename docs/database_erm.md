...
erDiagram
    ...
    CHARACTER_SHEETS ||--o{ CHARACTER_NOTES : has
...
## DnD Reference Data (read-only, seeded)
...
These tables are populated by `scripts/db-init.ts` from the `dnd_data_bundle/` JSON files. They are not modified by the application at runtime.
